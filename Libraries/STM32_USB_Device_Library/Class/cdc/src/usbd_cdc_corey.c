...
static uint8_t  *USBD_cdc_GetCfgDesc (uint8_t speed, uint16_t *length); // static
...
static void Handle_USBAsynchXfer (void *pdev)
{
  ...
  if(USB_Tx_State != 1)
  {
    ...
    if(APP_Rx_ptr_out == APP_Rx_ptr_in) 
    {
      USB_Tx_State = 0; 
			APP_Rx_ptr_out = 0;
			APP_Rx_ptr_in = APP_FOPS.pIf_DataTx(APP_Rx_Buffer,APP_RX_DATA_SIZE);    //~~~ !!!
      return;
    }
